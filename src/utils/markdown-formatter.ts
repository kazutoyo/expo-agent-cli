...
export function formatMarkdownForTerminal(markdown: string): string {
	...
	// Headers (# ## ### etc.)
	formatted = formatted.replace(/^### (.+)$/gm, (_, text: string) =>
	...
	formatted = formatted.replace(/\*\*(.+?)\*\*/g, (_, text: string) =>
		chalk.bold(text),
	);
	formatted = formatted.replace(/__(.+?)__/g, (_, text: string) =>
		chalk.bold(text),
	);
	...
	formatted = formatted.replace(/_(.+?)_/g, (_, text: string) =>
		chalk.italic(text),
	);
	...
	formatted = formatted.replace(
		/^- (.+)$/gm,
		(_, text: string) => `${chalk.yellow("●")}·${text}`,
	);
	formatted = formatted.replace(
		/^\* (.+)$/gm,
		(_, text: string) => `${chalk.yellow("●")}·${text}`,
	...
	formatted = formatted.replace(
		/^(\d+)\. (.+)$/gm,
		(_, num: string, text: string) => `${chalk.yellow(`${num}.`)}·${text}`,
	);
...
export function formatHighlight(text: string): string {
	...
	let formatted = text.replace(
		/<span class="algolia-docsearch-suggestion--highlight">(.+?)<\/span>/g,
		chalk.yellow("$1"),
	);
	// Handle <em> tags
