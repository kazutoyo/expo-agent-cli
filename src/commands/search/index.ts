...
import { cleanHighlight, formatHighlight } from '../../utils/markdown-formatter.js';
...
export const searchCommand = (program: Command, expoVersion: string) => {
  program
    ...
    .option('-l, --limit <number>', 'Number of results to show (default: 5, max: 20)', '5')
    ...
    .action(async (query: string, options: SearchOptions) => {
      try {
        ...
        if (options.pretty) {
          ...
          console.log(JSON.stringify({ query, total, results: formattedResults }, null, 2));
        }
      } catch (error) {
        console.error('Error searching documentation:', error instanceof Error ? error.message : error);
        process.exit(1);
      }
    });
};
